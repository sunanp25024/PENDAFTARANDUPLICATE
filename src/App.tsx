...
const App: React.FC = () => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 0: // Client, Posisi & Penempatan
        if (!formData.client) newErrors.client = 'Client harus dipilih';
        if (!formData.posisiDilamar) newErrors.posisiDilamar = 'Posisi yang dilamar harus dipilih';
        if (!formData.penempatan) newErrors.penempatan = 'Lokasi penempatan harus dipilih';
        if (!formData.detailPenempatan) newErrors.detailPenempatan = 'Detail penempatan harus dipilih';
        break;
